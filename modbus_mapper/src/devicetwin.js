...
class DeviceTwin {
    ...
    updateActual(deviceID, property, value) {
        ...
        reply_msg.twin = twin;
	if( this.qos == 0){
		this.mqttClient.publish(constant.defaultTopicPrefix + deviceID + constant.twinUpdateTopic, 
					JSON.stringify(reply_msg),{qos: 0, retain: false, dup: false});
	}else {
	...
	if(this.qos == 0){
		this.mqttClient.publish(constant.defaultTopicPrefix + deviceID + constant.twinGetTopic, 
					JSON.stringify(payload_msg), {qos: 0, retain: false, dup: false});
	}else {
    ...
    UpdateDirectActuals(devIns, deviceID, actualVals) {
		...
		if(this.qos == 0) {
		        this.mqttClient.publish(constant.defaultDirectTopicPrefix + deviceID + constant.directGetTopic, 
						JSON.stringify(directGetMsg), {qos: 0, retain: false, dup: false});
		}else {
                	this.mqttClient.publish(constant.defaultDirectTopicPrefix + deviceID + constant.directGetTopic, 
						JSON.stringify(directGetMsg));
		}
            });
        }
    }
